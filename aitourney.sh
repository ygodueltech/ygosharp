...
for deck1 in "$WINDBOW_HOME"/Decks/*.ydk; do
    ...
    for deck2 in "$WINDBOW_HOME"/Decks/*.ydk; do
        ...
        if [[ "$deck1" != "$deck2" ]]; then
            ...
            "$WINDBOW_HOME"/bin/Debug/WindBot.exe Name="$deck1" Host=127.0.0.1 Port=7911 Deck="$deck1" > /dev/null 2>&1 &
            "$WINDBOW_HOME"/bin/Debug/WindBot.exe Name="$deck2" Host=127.0.0.1 Port=7911 Debug=True Deck="$deck2" > duel.log
